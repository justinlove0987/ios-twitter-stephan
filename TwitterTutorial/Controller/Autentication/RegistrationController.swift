...
class RegistrationController: UIViewController {
    ...
    private let plusPhotoButton: UIButton = {
        ...
        button.addTarget(self, action: #selector(handleAnddProfilePhoto), for: .touchUpInside)
        return button
    }()
    ...
    @objc func handleAnddProfilePhoto() {
    ...
    @objc func handleRegistration() {
        ...
        AuthServic.shared.registerUser(credentials: credentials) { error, ref in

            guard let window = UIApplication.shared.windows.first(where: {$0.isKeyWindow}) else { return }

            guard let tab = window.rootViewController as? MainTabController else { return }

            tab.authenticateUsesrAndConfigureUI()
